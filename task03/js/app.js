...
http.createServer((req,res)=>{
  ...
  var str = path.split('/');
  var ipath = '../'+str.slice(2).join('/');
  var dirpath = __dirname+path;
  var dirpath1 = dirpath.split('.')[1];

  switch(dirpath1){
    case 'css':
      var data = fs.readFileSync(ipath);
...
//用户admin登录
  ...
  if(req.url==="/add"){
    req.on('data',function(data){
      ...
      var id = chapterList.length +1;
      var addch = {
          "chapterId":id,
          "chapterName":obj.title,
          "chapterContent":obj.content
      }
      //写入存储chapterList，断掉服务器后不存在addch中内容
      chapterList.push(addch);
    });
      res.end();
  }
